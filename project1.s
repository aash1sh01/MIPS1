...
Assign:	lb $a0, 0($s1)							
	j Checker
									#Load each character, with load byte and incerementing $s1 by 1 so that we jump to next character next, finally jmping to filter to check if the input is valid.
...
Checker: li $t1, 48							#checks if the values are valid and in range for our base.
	 ...
	 li $t5, 97
	 li $t6, 121
									#loading different values to registers to compare ASCII characters and filter if they are valid and also if the character is a number, lowercase or uppercase
	 blt $a0, $t1, Invalid											
...
Invalid: add $s0, $s0, $zero
	 j Last	
									#if invalid, we add zero to the total value then go to next character
End:	 li $v0, 1
